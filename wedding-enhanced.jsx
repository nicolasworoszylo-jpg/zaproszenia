  ...
  return (<section id="potwierdzenie" style={{padding:"80px 24px",background:`linear-gradient(170deg,${C.deep},${C.forest})`,scrollMarginTop:60,position:"relative"}}>
    ...
    <div style={{maxWidth:440,margin:"0 auto",background:`${C.warm}F0`,backdropFilter:"blur(10px)",border:`1px solid ${C.parch}`,padding:"36px 28px",boxShadow:"0 8px 36px rgba(0,0,0,.15)",position:"relative"}}>
      ...
      {att==="yes"&&(<><Reveal delay={.06}><div style={{display:"flex",gap:10,marginBottom:20}}><div style={{flex:1}}><label style={lSt}>Osoby</label><select value={g} onChange={e=>sG(e.target.value)} style={{...iSt,appearance:"auto"}}>{[1,2,3,4,5].map(x=><option key={x} value={x}>{x}</option>)}</select></div><div style={{flex:1}}><label style={lSt}>Autobus</label><select value={bus} onChange={e=>sB(e.target.value)} style={{...iSt,appearance:"auto"}}><option value="no">Nie potrzebuję</option><option value="yes">Tak, poproszę</option></select></div></div></Reveal>
        ...
        <Reveal delay={.14}><div style={{marginBottom:20}}><label style={lSt}>Alergie / uwagi</label><input type="text" value={diet} onChange={e=>sD(e.target.value)} placeholder="Opcjonalnie - np. uczulenie na orzechy" style={iSt} onFocus={e=>e.target.style.borderColor=C.gold} onBlur={e=>e.target.style.borderColor=C.parch}/><p style={{fontFamily:F.body,fontSize:11,color:C.stone,marginTop:6,lineHeight:1.4,opacity:.8}}>Pole opcjonalne. Wpisanie informacji o alergiach oznacza wyraźną zgodę na przetwarzanie danych o zdrowiu zgodnie z art. 9 ust. 2 lit. a RODO - wyłącznie w celu organizacji posiłku.</p></div></Reveal></>)}
...
function AudioPlayer(){
  ...
  const[hint,setHint]=useState(true);  // hint pulsuje pierwsze 8s zeby przyciagnac wzrok
  useEffect(()=>{const t=setTimeout(()=>setHint(false),8000);return()=>clearTimeout(t)},[]);
  if(!CONFIG.bgMusicUrl)return null;
  const toggle=()=>{
    const a=audioRef.current;if(!a){console.warn("[audio] ref null");return}
    setHint(false);
    if(playing){a.pause();setPlaying(false)}
    else{a.volume=0.35;a.play().then(()=>setPlaying(true)).catch(e=>console.warn("[audio] play blocked:",e))}
  };
  return (<div style={{position:"fixed",bottom:24,right:24,zIndex:9999,display:"flex",alignItems:"center",gap:10,flexDirection:"row-reverse",pointerEvents:"auto"}}>
    <audio ref={audioRef} src={CONFIG.bgMusicUrl} loop preload="auto" crossOrigin="anonymous" playsInline/>
    <button onClick={toggle} onMouseEnter={()=>setExpanded(true)} onMouseLeave={()=>setExpanded(false)} aria-label={playing?"Wycisz muzykę":"Włącz muzykę"} title={playing?"Wycisz muzykę":"Włącz muzykę"} style={{width:56,height:56,borderRadius:"50%",border:`2px solid ${C.gold}`,background:`${C.deep}F2`,backdropFilter:"blur(12px)",WebkitBackdropFilter:"blur(12px)",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:hint?`0 0 0 0 ${C.gold}80, 0 6px 24px rgba(0,0,0,.35)`:`0 6px 24px rgba(0,0,0,.35)`,transition:`all .4s ${SPRING}`,transform:expanded?"scale(1.08)":"scale(1)",animation:hint?"audioPulse 1.6s ease-out infinite":"none"}}>
      {playing
        ? <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={C.gold} strokeWidth="2"><rect x="6" y="4" width="4" height="16" rx="1"/><rect x="14" y="4" width="4" height="16" rx="1"/></svg>
        : <svg width="22" height="22" viewBox="0 0 24 24" fill={C.gold} stroke={C.gold} strokeWidth="1.5"><polygon points="7,4 20,12 7,20"/></svg>
      }
    </button>
    {(expanded||hint)&&CONFIG.bgMusicTitle&&(<div style={{background:`${C.deep}F2`,backdropFilter:"blur(12px)",WebkitBackdropFilter:"blur(12px)",border:`1px solid ${C.gold}40`,borderRadius:10,padding:"10px 16px",whiteSpace:"nowrap",boxShadow:`0 4px 16px rgba(0,0,0,.3)`,animation:`modalIn .25s ${EASE_OUT_EXPO}`}}>
      <p style={{fontFamily:F.body,fontSize:11,color:C.gold,letterSpacing:2,textTransform:"uppercase",margin:0}}>{hint&&!playing?"Włącz muzykę":CONFIG.bgMusicTitle}</p>
    </div>)}
  </div>);
}
