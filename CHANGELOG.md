...
- **Fixed**: `index.html` formularz checkout - usunięto `defer` ze `<script src="vendor/supabase.min.js">` który powodował cichy TypeError w inline script (window.supabase=undefined w momencie createClient) → cały skrypt po linii 3384 nie wykonywał się → submit handler, toggle "Wpisuję teraz", walidacja kodu rabatowego cicho przestawały działać. Formularz submitował się defaultowo i czyścił pola. Dodatkowo: natywny radio input pełnie ukryty (appearance:none + clip-path), hardcoded kod HIPERFIKSACJA=99% z live update ceny w submit buttonie. Setup po stronie Stripe: HIPERFIKSACJA_STRIPE_SETUP.md _(2026-05-14)_
